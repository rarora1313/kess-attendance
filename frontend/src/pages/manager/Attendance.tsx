...
import { getAttendance, correctAttendance } from '../../api';
...
export default function ManagerAttendance() {
  ...
  const { data: records = [] } = useQuery({
    queryKey: ['attendance', date, user?.branchId],
    queryFn: () => getAttendance({ branchId: user?.branchId, date }),
  });
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Employee', 'Sign In', 'In Photo', 'Sign Out', 'Out Photo', 'Break', 'Hours', 'Flags', ''].map(h => (
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {records.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400">No records for this date.</td></tr>
              )}
            </tbody>
          </table>
        </div>
