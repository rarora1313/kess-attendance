                ...
                <select className={inputCls} value={form.employeeId}
                  ...
                  {employees.filter((e: any) => e.status === 'active').map((e: any) => (
                    <option key={e.id} value={e.id}>
                      {e.firstName} {e.lastName} — {e.branch?.name}
                    </option>
                  ))}
                </select>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Employee', 'Branch', 'Date', 'Task', 'Description', 'Hours', 'Status', 'Actions'].map(h => (
            ...
            <tbody className="divide-y divide-gray-100">
              {timesheets.map((t: any) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 text-gray-500">{t.employee.branch?.name ?? '—'}</td>
              ...
              {timesheets.length === 0 && (
                <tr><td colSpan={8} className="px-6 py-8 text-center text-gray-400">No timesheets yet. Click "+ New Timesheet" to add one.</td></tr>
              )}
            </tbody>
          </table>
        </div>
